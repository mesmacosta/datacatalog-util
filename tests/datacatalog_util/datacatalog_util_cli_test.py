...
class TagManagerCLITest(unittest.TestCase):
    ...
    def test_parse_args_create_tags_should_parse_mandatory_args(self):
        args = datacatalog_util_cli.DatacatalogUtilsCLI._parse_args(['create-tags', '--csv-file', 'test.csv'])
    ...
    def test_run_create_tags_should_call_tag_creator(self, mock_tag_datasource_processor):
        datacatalog_util_cli.DatacatalogUtilsCLI.run(['create-tags', '--csv-file', 'test.csv'])
    ...
    @mock.patch(
        'datacatalog_util.datacatalog_util_cli.tag_template_datasource_processor.'
        'TagTemplateDatasourceProcessor')
    def test_run_create_tag_templates_should_call_tag_template_creator(
            self,
            mock_tag_template_datasource_processor):
        datacatalog_util_cli.DatacatalogUtilsCLI.run(['create-tag-templates', '--csv-file', 'test.csv'])
    ...
    def test_run_delete_tag_templates_should_call_correct_method(
            self,
            mock_tag_template_datasource_processor):
        datacatalog_util_cli.DatacatalogUtilsCLI.run(['delete-tag-templates', '--csv-file', 'test.csv'])
    ...
    @mock.patch(
        'datacatalog_util.datacatalog_util_cli.tag_template_datasource_exporter.'
        'TagTemplateDatasourceExporter')
    def test_run_export_tag_templates_should_call_correct_method(
            self,
            mock_tag_template_datasource_exporter):
        datacatalog_util_cli.DatacatalogUtilsCLI.run(['export-tag-templates',
                                           '--file-path', 'test.csv',
                                           '--project-ids', 'my-project1,my-project2'
                                           ])
    ...
    def test_run_export_tags_should_call_correct_method(
            self,
            mock_tag_datasource_exporter):
        datacatalog_util_cli.DatacatalogUtilsCLI.run(['export-tags',
                                           '--dir-path', 'test.csv',
                                           '--project-ids', 'my-project1,my-project2'
                                           ])
